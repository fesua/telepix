"""Run the 3-config sweep TWICE with histogram matching applied two different ways:

  Mode 1 — histmatch_tif:    apply match_histograms on full Planet TIFs (UInt16 RGB),
                              modify the input2048 stage. Pipeline downstream sees
                              already-color-corrected images.
  Mode 2 — histmatch_tensor: keep input2048 standard. After preprocessing, walk every
                              cropped uint8 256×256 TIF and apply match_histograms
                              against the reference crop. Modifies output256/.

Both modes use 0010 (2020-03-08 d2) as the reference. Output goes to:
  outputs/run_<TS>_ctx_<v0>-<v1>-<v2>_histmatch_<mode>/
"""
...
MODES = ["histmatch_tif", "histmatch_tensor"]
...
def patch_view_map(target_script: Path, third: str) -> list:
    ...
    src = re.sub(r"VIEW_MAP\s*=\s*\[.*?\n\]\n", new_map, src, count=1, flags=re.DOTALL)
    target_script.write_text(src)
    return slots


def get_full_view_and_rpc(planet_id: str, mode: str = "baseline", slot: int | None = None):
    """If mode == 'histmatch_tif', read the matched input2048_histmatch_tif/ image
    (already uint8 + percentile-stretched) so gt.png is in the same color space as
    the model's render."""
    from osgeo import gdal
    from preprocess.rpc_model import RPCModel
    gdal.UseExceptions()
    stem = ALL_STEMS[planet_id]
    if mode == "histmatch_tif" and slot is not None:
        rgb_no = ["001", "002", "003", "004", "005"][slot]
        matched_tif = REPO / f"CreatDataset/example/input2048_histmatch_tif/test/image/{slot}/JAX_Tile_999_RGB_{rgb_no}.tif"
        if matched_tif.exists():
            ds = gdal.Open(str(matched_tif), gdal.GA_ReadOnly)
            arr = ds.ReadAsArray()  # (3, h, w) uint8
            img_hwc = arr.transpose(1, 2, 0)
            h, w = img_hwc.shape[:2]
            ds = None
            # Build RPC from the original RPC TXT (matched TIF has it embedded but easier to re-read)
            src_rpc_txt = REPO / "Dataset" / f"{stem}_basic_analytic_RPC.TXT"
            raw = {}
            for line in src_rpc_txt.read_text().splitlines():
                k, _, v = line.strip().partition(":")
                if k: raw[k.strip()] = v.strip()
            rpc_dict = {
    ...
    # baseline / fallback: read original Planet TIF and percentile-stretch
...
def stitch_into_view(planet_id, slot, raw_dir, output_root, mode="baseline"):
    gt_full, rpc, fh, fw = get_full_view_and_rpc(planet_id, mode=mode, slot=slot)
    render_canvas = np.zeros((fh, fw, 3), dtype=np.uint8)
    depth_canvas = np.full((fh, fw), np.nan, dtype=np.float32)
    rgb_no = ["001", "002", "003", "004", "005"][slot]
    color_dir = raw_dir / "renders/re10k/JAX_Tile_999/color"
    height_dir = raw_dir / "renders/re10k/JAX_Tile_999/height"
    ...
    return gt_full[by0:by1, bx0:bx1], render_canvas[by0:by1, bx0:bx1], depth_canvas[by0:by1, bx0:bx1]
...
def colormap_depth(depth):
    ...
    vmin = float(np.nanmin(depth)); vmax = float(np.nanmax(depth))
...
def build_full_ply(raw_dir: Path, out_path: Path):
    from plyfile import PlyData, PlyElement
    npz_files = sorted((raw_dir / "renders/re10k/JAX_Tile_999/gaussians").glob("*.npz"))
    if not npz_files:
        return {"gaussians_count": 0, "z_max_m": None}
    ...
    means = np.concatenate(all_means); scales = np.concatenate(all_scales)
    rots = np.concatenate(all_rots);   opac = np.concatenate(all_opac)
    ...
    N = means.shape[0]; n_rest = f_rest.shape[1]
    rec = np.concatenate([means.astype(np.float32), normals, f_dc, f_rest,
                          logit_opac.reshape(-1, 1), log_scales, rots.astype(np.float32)], axis=1)
...
for mode in MODES:
    for third in THIRD_CTX:
        print(f"\n{'='*70}\nMODE={mode}  CONFIG ctx = 0012, 0018, {third}\n{'='*70}")

        # 1. Patch VIEW_MAP in BOTH converter scripts (so histmatch_crops_tensor can find ref slot)
        patch_view_map(SCRATCH / "convert_planet.py",                 third)
        patch_view_map(SCRATCH / "convert_planet_histmatch_tif.py",   third)

        # 2. Clean working dirs
        run("rm -rf CreatDataset/example/input2048/test/image/*/*.tif "
            "CreatDataset/example/input2048/test/height/*/*.tif "
            "CreatDataset/example/input2048_histmatch_tif/test/image/*/*.tif "
            "CreatDataset/example/input2048_histmatch_tif/test/height/*/*.tif "
            "CreatDataset/example/output256/test/* 2>/dev/null", cwd=str(REPO))

        # 3. Convert with the mode-appropriate script
        if mode == "histmatch_tif":
            run(f"python {SCRATCH/'convert_planet_histmatch_tif.py'}", cwd=str(REPO))
            input_folder = "./example/input2048_histmatch_tif"
        else:
            run(f"python {SCRATCH/'convert_planet.py'}", cwd=str(REPO))
            input_folder = "./example/input2048"

        # 4. Preprocess (3-view-common + fixup + minmax)
        run(f"python satellite_sfm_crop2048to256.py "
            f"--input_folder {input_folder} --output_folder ./example/output256 "
            f"--splits test --disable_srtm4 --view-mode fixed >/dev/null 2>&1",
        ...
        # 5. For histmatch_tensor mode, apply match on the cropped uint8 TIFs now
        if mode == "histmatch_tensor":
            out = run(f"python {SCRATCH/'histmatch_crops_tensor.py'}", cwd=str(REPO), capture=True)
            for line in out.splitlines():
                if "slot" in line or "Reference" in line:
                    print(f"  {line}")

        # 6. Inference
        ts = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        raw_dir = REPO / f"outputs/_raw_{ts}_ctx_0012-0018-{third}_{mode}"
        ...
        # 7. Repackage to clean layout
        clean_dir = REPO / f"outputs/run_{ts}_ctx_0012-0018-{third}_{mode}"
        clean_dir.mkdir(parents=True, exist_ok=True)
        output_root = REPO / "CreatDataset/example/output256/test"
        heights_info = {}
        used = set(FIXED + [third]); rem = [v for v in ALL_STEMS if v not in used]
        ...
        for slot, pid in enumerate(all_planet_ids):
            ...
            result = stitch_into_view(pid, slot, raw_dir, output_root, mode=mode)
        ...
        heights_info["_histmatch_mode"] = mode
        heights_info["_histmatch_reference"] = "20200308_021307_ssc2d2_0010"
