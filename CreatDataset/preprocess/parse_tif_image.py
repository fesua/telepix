...
def parse_tif_rpc_image(tiff_fpath,rpc170_path):
    ...
    rpc_data = {}
    rpc_data['LINE_OFF'] = float(data[0])
    rpc_data['SAMP_OFF'] = float(data[1])
    rpc_data['LAT_OFF'] = float(data[2])
    rpc_data['LONG_OFF'] = float(data[3])
    rpc_data['HEIGHT_OFF'] = float(data[4])
    rpc_data['LINE_SCALE'] = float(data[5])
    rpc_data['SAMP_SCALE'] = float(data[6])
    rpc_data['LAT_SCALE'] = float(data[7])
    rpc_data['LONG_SCALE'] = float(data[8])
    rpc_data['HEIGHT_SCALE'] = float(data[9])

    rpc_data['LINE_NUM_COEFF'] = np.asarray(data[10:30], dtype=np.float64).tolist()
    rpc_data['LINE_DEN_COEFF'] = np.asarray(data[30:50], dtype=np.float64).tolist()
    rpc_data['SAMP_NUM_COEFF'] = np.asarray(data[50:70], dtype=np.float64).tolist()
    rpc_data['SAMP_DEN_COEFF'] = np.asarray(data[70:90], dtype=np.float64).tolist()
    ...
    rpc_dict = {
        ...
        'rowNum': rpc_data['LINE_NUM_COEFF'],
        'rowDen': rpc_data['LINE_DEN_COEFF'],
        'colNum': rpc_data['SAMP_NUM_COEFF'],
        'colDen': rpc_data['SAMP_DEN_COEFF']
    }
