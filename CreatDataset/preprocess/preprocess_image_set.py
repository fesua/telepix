...
from preprocess.parse_tif_image import parse_tif_image,parse_tif_rpc_image
...
def _preprocess_single_tif(args):
    ...
    img_tif_path = os.path.join(tif_image_folder, item)
    rpc170_folder = img_tif_path.replace("images","rpcs").replace('.tif', '_170.rpc')
    img, meta_dict = parse_tif_rpc_image(img_tif_path,rpc170_folder)
