...
class EncoderCostVolume(Encoder[EncoderCostVolumeCfg]):
    ...
    def forward(
        ...
        depth_min,depth_max = context['hei_min_max'][:,:,0] , context['hei_min_max'][:,:,1]
        near,far = torch.ones((b, v), dtype=torch.float32, device=device)*depth_min, torch.ones((b, v), dtype=torch.float32, device=device)*depth_max
        ...
        heights, densities, raw_gaussians = self.depth_predictor(
            in_feats,
        ...
        if visualization_dump is not None:
            visualization_dump["depth"] = rearrange(
                heights, "b v (h w) srf s -> b v h w srf s", h=h, w=w
            )
