...
def geodetic2ecef_torch(lat, lon, alt, ell):
    """
    lat, lon, alt: torch.Tensor, shape [...], in degrees
    ell: namedtuple with semimajor_axis a, semiminor_axis b
    returns: x, y, z, same shape
    """
    ...
    N = a**2 / torch.sqrt((a * torch.cos(lat))**2 + (b * torch.sin(lat))**2)

    x = (N + alt) * torch.cos(lat) * torch.cos(lon)
    y = (N + alt) * torch.cos(lat) * torch.sin(lon)
    z = (N * (b / a) ** 2 + alt) * torch.sin(lat)
    return x, y, z

def ecef2enu_torch(x, y, z, lat0, lon0):
    """
    x, y, z: target ECEF coordinates
    lat0, lon0: observer geodetic coords (degrees)
    returns: e, n, u in meters
    """
    lat0 = torch.deg2rad(lat0)
    lon0 = torch.deg2rad(lon0)

    t = torch.cos(lon0) * x + torch.sin(lon0) * y
    e = -torch.sin(lon0) * x + torch.cos(lon0) * y
    u = torch.cos(lat0) * t + torch.sin(lat0) * z
    n = -torch.sin(lat0) * t + torch.cos(lat0) * z
    return e, n, u

def geodetic2enu_torch(lat, lon, alt, lat0, lon0, alt0, ell):
    # ECEF of target
    x, y, z = geodetic2ecef_torch(lat, lon, alt, ell)
    # ECEF of observer
    x0, y0, z0 = geodetic2ecef_torch(lat0, lon0, alt0, ell)
    # delta ECEF
    dx, dy, dz = x - x0, y - y0, z - z0
    # convert to ENU
    e, n, u = ecef2enu_torch(dx, dy, dz, lat0, lon0)
    return e, n, u
